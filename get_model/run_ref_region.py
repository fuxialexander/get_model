...
class ReferenceRegionDataModule(GETDataModule):
    ...
    def build_from_zarr_dataset(self, zarr_dataset):
        return ReferenceRegionDataset(
            self.reference_region_motif,
            zarr_dataset,
            quantitative_atac=self.cfg.dataset.quantitative_atac,
            sampling_step=self.cfg.dataset.sampling_step,
        )

    def build_inference_reference_region_dataset(self, zarr_dataset):
        return InferenceReferenceRegionDataset(
            self.reference_region_motif,
            zarr_dataset,
            quantitative_atac=self.cfg.dataset.quantitative_atac,
            sampling_step=self.cfg.dataset.sampling_step,
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            self.dataset_train = self.build_from_zarr_dataset(self.dataset_train)
        ...
        if stage == "predict":
            if self.cfg.task.test_mode == "predict":
                ...
                self.cfg.task.test_mode == "inference"
                or self.cfg.task.test_mode == "interpret"
                ...
                perturb_mode = (
                    "mutation"
                    if "mutation" in self.cfg.task.test_mode
                    else "peak_inactivation"
                )
                self.dataset_predict = self.build_perturbation_inference_dataset(
                    self.dataset_predict.inference_dataset,
                    self.mutations,
                    mode=perturb_mode,
                )
        if stage == "validate":
...
class RegionLitModel(LitModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if "abc" in obs:
            idx = np.random.choice(obs["abc"].flatten().shape[0], 1000, replace=False)
            obs["abc"] = obs["abc"].flatten()[idx]
            pred["abc"] = pred["abc"].flatten()[idx]

        if "motif" in obs:
            # random draw 1 sample, discard the rest
            idx = np.random.choice(obs["motif"].shape[0], 1, replace=False)
                ...
                if key != "hic":
                    ...
                    for i in range(len(pred[key])):
                        ...
                        if self.cfg.run.use_wandb:
                            self.logger.experiment.log(
                                {
                                    f"heatmap_{key}_pred": wandb.Image(
                                        sns.heatmap(
                                            pred[key][i]
                                            .detach()
                                            .cpu()
                        ...
                        if self.cfg.run.use_wandb:
                            self.logger.experiment.log(
                                {
                                    f"heatmap_{key}_obs": wandb.Image(
                                        sns.heatmap(
                                            obs[key][i]
                                            .detach()
                                            .cpu()
    ...
    def predict_step(self, batch, batch_idx, *args, **kwargs):
        if self.cfg.task.test_mode == "inference":
            ...
            for batch_element in range(len(batch["gene_name"])):
                ...
                goi_idx = goi_idx[
                    goi_idx < batch["region_motif"][batch_element].shape[0]
                ]  # filter out TSS that exceed number of regions
                if len(goi_idx) == 0:
                    ...
                    result_df.append(
                        {
                            "gene_name": gene_name,
                            "key": key,
                            "pred": pred[key][batch_element][:, strand][goi_idx]
                            .max()
                            .cpu()
                            .item(),
            ...
            for i in range(batch_size):
                ...
                result = {
                    ...
                    "pred_wt": pred["pred_wt"]["exp"][i][:, strand][goi_idx]
                    .mean()
                    .cpu()
                    .item(),
                    "pred_mut": pred["pred_mut"]["exp"][i][:, strand][goi_idx]
                    .mean()
                    .cpu()
                    .item(),
                    "obs": pred["obs_wt"]["exp"][i][:, strand][goi_idx]
                    .mean()
                    .cpu()
                    .item(),
                }
            ...
            pred = np.array(
                [
                    pred["exp"][i][:, batch["strand"][i].cpu().numpy()][
                        batch["all_tss_peak"][i].cpu().numpy()
                    ].mean()
            ...
            obs = np.array(
                [
                    obs["exp"][i][:, batch["strand"][i].cpu().numpy()][
                        batch["all_tss_peak"][i].cpu().numpy()
                    ].mean()
                    for i in range(len(batch["gene_name"]))
                ]
        ...
        if self.cfg.finetune.use_lora:
            ...
            if self.cfg.stage == "fit":
                ...
                if self.cfg.finetune.lora_checkpoint is not None:
                    ...
                    load_state_dict(
                        model, lora_state_dict, strict=self.cfg.finetune.strict
                    )
    ...
    def on_validation_epoch_end(self):
        ...
        metric_dict["motif_scaler"] = (
            self.cfg.dataset.reference_region_motif.motif_scaler
        )
        ...
        if not os.path.exists(f"{self.cfg.machine.output_dir}/val_metrics.csv"):
            df.to_csv(f"{self.cfg.machine.output_dir}/val_metrics.csv", index=False)
        else:
            df.to_csv(
                f"{self.cfg.machine.output_dir}/val_metrics.csv",
                mode="a",
                header=False,
                index=False,
            )
